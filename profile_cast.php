...
$sql = mysql_query("SELECT  *, Left(profile,130) as Profile1,Status,date_format(Lastlogin,'%d-%M-%Y') as lastlogindate,Photoprotect FROM register WHERE Caste='$Strcaste'  AND Status <> 'InActive' LIMIT $from, $max_results "); 
      ...
      <table width="97%" height="120" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td background="pics/search-mid.gif"><table width="560" border="0" align="center" cellpadding="2" cellspacing="2">
            <tr>
              <td width="193" class="bodyg"><div align="left"><strong>Caste : </strong><strong><?php echo $Strcaste; ?></strong></div></td>
...
$total_results  = mysql_result(mysql_query("SELECT  COUNT(*) as Num FROM register where  Caste='$Strcaste' AND Status <> 'InActive' "),0); 
...
//echo  " &nbsp;[ <A HREF=\"".$_SERVER['PHP_SELF']."?page=" . $i . "&StrGender=" .urlencode($StrGender) . "&StrSAge=" .urlencode($StrSAge) .  "&StrEAge=" .urlencode($StrEAge) .  "&StrCaste=" .urlencode($Strcaste) . "&StrCaste=" .urlencode($StrCaste) . "&StrPhoto=" .urlencode($StrPhoto) . "\">$i</A>  ] ";
