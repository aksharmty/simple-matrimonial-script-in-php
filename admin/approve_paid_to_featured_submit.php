...
if(!$_POST['chkapprove'])
{
			...
			header('location:approve_paid_to_featured.php?Action=not check');
}


	foreach($_POST["chkapprove"] AS $val)
	{
	// UPDATE STATUS from Paid To Featured
	$update1 = mysql_query("update register set status='Featured' where matriid IN ('$val')")
